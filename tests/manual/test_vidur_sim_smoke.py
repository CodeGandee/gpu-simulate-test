...
def main() -> None:
    parser = argparse.ArgumentParser(description="Manual smoke: vidur-sim (profiling required)")
    ...
    cmd = [
        sys.executable,
        "-m",
        "gpu_simulate_test.cli.vidur_sim",
    ...
    if not (req_csv.exists() and tok_csv.exists() and meta_json.exists()):
