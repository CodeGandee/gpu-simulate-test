...
- In this repo’s current Vidur wrapper (`src/gpu_simulate_test/vidur_ext/sim_runner.py`), CPU overhead modeling is disabled by default (`scenario.vidur.enable_cpu_overhead_modeling=false`, which maps to Vidur’s internal `skip_cpu_overhead_modeling=True`), making “what Vidur measures” closer to “GPU + modeled scheduler time” than full system time.
...
- CPU overhead modeling is disabled by default: we run with `scenario.vidur.enable_cpu_overhead_modeling=false` (Vidur internal `skip_cpu_overhead_modeling=True`), and the Vidur profiling bundle vendored in this repo may not include `cpu_overheads.csv`. That host/runtime overhead can dominate batch=1 latency and is a common source of underprediction.
