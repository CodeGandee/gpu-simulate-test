...
modeling (`skip_cpu_overhead_modeling=true` in the execution-time predictor config; in this repo we expose the same knob as `scenario.vidur.skip_cpu_overhead_modeling=true`).
...
`skip_cpu_overhead_modeling=true` (repo knob: `scenario.vidur.skip_cpu_overhead_modeling=true`). If you need CPU overhead fidelity for a specific framework/runtime, use:
